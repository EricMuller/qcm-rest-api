...
$KCADM update clients/$ID -r $REALM_NAME -s name="qcm-web-dev" -s protocol=openid-connect -s publicClient=true -s standardFlowEnabled=true -s 'redirectUris=["http://127.0.0.1:4200/*"]' -s baseUrl="http://127.0.0.1:4200/" -s 'webOrigins=["*"]'

CLIENT_ID=qcm-rest-api
ID=$(createClient $REALM_NAME $CLIENT_ID)
$KCADM update clients/$ID -r $REALM_NAME -s name="qcm-rest-api" -s protocol=openid-connect -s bearerOnly=true -s standardFlowEnabled=false -s 'redirectUris=["http://127.0.0.1:4200/*"]' -s baseUrl="http://127.0.0.1:4200/" -s 'webOrigins=["*"]'

#users
USER_NAME=webdev
USER_ID=$(createUser $REALM_NAME $USER_NAME)
$KCADM update users/$USER_ID -r $REALM_NAME -s firstName="webdev" -s lastName="webdev" -s email="webdev@webmarks.net"

...
import com.emu.apps.qcm.application.importation.FileService;
...
import com.emu.apps.qcm.rest.controllers.domain.hal.UploadActionModelAssembler;
import com.emu.apps.qcm.rest.controllers.domain.hal.UploadModelAssembler;
...
import static com.emu.apps.qcm.rest.controllers.domain.DomainMappings.UPLOADS;
...
@RestController
@RequestMapping(value = DOMAIN_API + UPLOADS, produces = APPLICATION_JSON_VALUE)
@Tag(name = "Upload")
@Timed("uploads")
@Slf4j
public class UploadRestController {
    ...
    public UploadRestController(UploadRepository uploadRepository, QcmResourceMapper qcmResourceMapper, UploadModelAssembler uploadModelAssembler, UploadActionModelAssembler uploadActionModelAssembler, FileService fileService) {
    ...
    public PagedModel <EntityModel <ActionResource>> getActions(@PathVariable("uuid") String uploadUuid,
        ...
        var actions = List.of(new ActionResource(uploadUuid, Actions.IMPORT, "File Import"));
    ...
    @ResponseBody
    @PutMapping(value = "/{uuid}/actions/" + Actions.IMPORT + "/invoke")
    ...
    public UploadResource importFileByUploadUuid(@PathVariable("uuid") String uploadUuid) {
        ...
        try {
            Future <Upload> uploadCompletableFuture = fileService.importFileAsync(new UploadId(uploadUuid), PrincipalId.of(getPrincipal()));
